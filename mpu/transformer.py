...
class GPT2ParallelSparseSelfAttention(torch.nn.Module):
    ...
    def get_ops(self, L):
        if L not in self.__class__.ops:
            ...
            layout = self.__class__._make_layout(
                self.num_attention_heads_per_partition, L // sparsity.block, sparsity.mode,
            ...
            self.__class__.ops[L] = (sparse_dot_sdd_nt, sparse_dot_dsd_nn, sparse_softmax)
    ...
    def __init__(self, hidden_size, num_attention_heads,
        ...
        super(GPT2ParallelSparseSelfAttention, self).__init__()
        ...
        sparsity.mode = 'fixed'
        sparsity.unidirectional = True
        sparsity.block = 16
        # TODO: move to config
        sparsity.stride = 1024
    ...
    def forward(self, hidden_states, ltor_mask):
        ...
        sparse_dot_sdd_nt, sparse_dot_dsd_nn, sparse_softmax = self.get_ops(tgt_len)
        ...
        shape_for_sparse = (bsz, self.num_attention_heads_per_partition, tgt_len, self.hidden_size_per_attention_head)
        query_layer = query_layer.view(*shape_for_sparse)
        key_layer = key_layer.view(*shape_for_sparse)
        value_layer = value_layer.view(*shape_for_sparse)
        # Raw attention scores. [b, np, s, s]
        norm_factor = 1.0 / math.sqrt(self.hidden_size_per_attention_head)
        query_layer *= norm_factor
        attention_scores = sparse_dot_sdd_nt(query_layer, key_layer)
        attention_probs = sparse_softmax(
            attention_scores,
            key_padding_mask=ltor_mask.view(tgt_len, tgt_len),
            attn_mask=ltor_mask.view(tgt_len, tgt_len),
        ...
        context_layer = sparse_dot_dsd_nn(attention_probs, value_layer)
        # [b, s, hp]
        context_layer = context_layer.view(
            bsz, tgt_len, self.hidden_size_per_partition)
...
class GPT2ParallelTransformerLayer(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 layernorm_epsilon,
                 init_method,
                 output_layer_init_method=None, use_sparse=False):
        ...
        # Self attention.
        if use_sparse:
            self.attention = GPT2ParallelSparseSelfAttention(
                hidden_size,
                num_attention_heads,
                attention_dropout_prob,
                output_dropout_prob,
...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 use_scaled_init_for_output_weights=True, use_sparse=False):
        ...
        def get_layer(idx):
            return GPT2ParallelTransformerLayer(
                ...
                output_layer_init_method=output_layer_init_method,
                use_sparse=(1 - idx % 2) and use_sparse
            )
